...
GetCI_Sparse = function(vars, outGrid = NULL, level = 0.95, R = 999, userBwMu = NULL, userBwCov = NULL,
  ...
  sparsity = sapply(vars[sapply(vars, is.list)], function(v) {
    optns = fdapace::SetOptions(v[['Ly']], v[['Lt']], optns = NULL)
    optns$dataType
    })
  if("Sparse" %in% sparsity){
    sparsity = "Sparse"
  }else{
    sparsity = "Dense"
  }
  ...
  if(sparsity == "Sparse"){
    ...
    u95 = u
    if(is.null(outGrid)){
      outGrid = seq(l, u, length.out=51)
    }else{
      if(min(outGrid)<l | max(outGrid)>u){
        stop('The input "outGrid" is out of the range of Lt, the list of time points in the "vars".')
      }
    }
  }
